...
public class UserInterface implements UserInterfaceAPI {
    ...
    private void printNewName() {
        printLine(
                "scrabble.Player "
                        + scrabble.getCurrentPlayer().getPrintableId()
                        + " is now named "
                        + scrabble.getCurrentPlayer().getName()
                        + ".");
    }
    ...
    private void printPrompt() {
        ...
        printTiles(); // suppressed for the scrabble.Bots
    }
    ...
    private void printPoolSize() {
        printLine("scrabble.Pool has " + scrabble.getPool().size() + " tiles");
    }

    private void printHelp() {
        printLine(
                "Command options: Q (quit), N (name), P (pass), X (exchange), C (challenge), S (scores), O (pool) or play");
        printLine("Names must begin with a letter.");
        printLine("For an exchange, enter the letters that you wish to exchange. E.g. X ABC");
        printLine(
                "For a play, enter the grid reference of the first letter, A (across) or D (down), "
                        + " and the word including any letters already on the board. E.g. A1 D HELLO");
        printLine(
                "For a play with a blank, enter the grid reference of the first letter, A (across) or D (down),"
    ...
    private void printPlayError(int errCode) {
        ...
        switch (errCode) {
            case Board.WORD_OUT_OF_BOUNDS:
                message = "Error: scrabble.Word does not fit on the board.";
                break;
